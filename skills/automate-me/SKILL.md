...
Use `writing-for-agent` skill to author the skill. Placement:
...
Apply the **unslop** skill and `writing-for-agent`'s writing guidelines to every line.
...
A `-mode` skill is subjective output. A `writing-for-agent`-style test/iterate benchmark loop isn't useful here. Vibe-check with the user: does it read like them? Did it miss anything? Then ship.

Run a description-optimization loop only if the skill's trigger accuracy turns out to be a problem in practice.

## When not to use

- User wants a task-specific skill (not working conventions): `writing-for-agent` alone, no mining required.
